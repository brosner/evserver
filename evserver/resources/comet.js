...
comet_restart_timeout = 5000;       // 5 seconds
...
function schedule_connection_sse(url, callback) {
    ...
    es.addEventListener('comet',   event_callback, false);
    es.addEventListener('orbited', event_callback, false);
    es.addEventListener('ping', event_callback, false);


    return function () {
        if(es){
            es.removeEventListener('comet',   event_callback, false);
            es.removeEventListener('orbited', event_callback, false);
...
function comet_connection(url, user_callback_o, transport_local) {
   ...
   comet_lo g('asd');
    comet_log('comet: using transport:' + transport +'  restart_timeout:' + comet_restart_timeout + ' keepalive_timeout:' + comet_keepalive_timeout);
    connect_function = transports[transport];
